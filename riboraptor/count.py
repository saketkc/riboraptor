...
def read_enrichment(read_lengths, min_length=28, max_length=32):
    ...
    if ".gz" in read_lengths:
        read_lengths_df = pd.read_table(
            read_lengths, names=["frag_len", "frag_cnt"], sep="\t", compression="gzip"
        )
    else:
        read_lengths_df = pd.read_table(
    ...
    read_lengths = pd.Series(
        read_lengths_df.frag_cnt.tolist(), index=read_lengths_df.frag_len.tolist()
    )
...
def get_bam_coverage(bam, bed, outprefix=None):
    ...
    with tqdm(total=total_counts) as pbar:
        for read in bam.fetch():
            ...
            coverage["{}:{}:5prime".format(read.reference_name, position_5prime)][
                query_length
            ]["+"] += 0
            coverage["{}:{}:5prime".format(read.reference_name, position_5prime)][
                query_length
            ]["-"] += 0
            coverage["{}:{}:3prime".format(read.reference_name, position_3prime)][
                query_length
            ]["+"] += 0
            coverage["{}:{}:3prime".format(read.reference_name, position_3prime)][
                query_length
    ...
    if outprefix:
        ...
        df["gene_strand"] = df.apply(
            lambda row: _get_gene_strand(
                bed_tree, row["chrom"], row["start"], row["end"]
            ),
            axis=1,
        )
        df = df.sort_values(
        ...
        df.to_csv("{}.tsv".format(outprefix), sep="\t", index=False, header=True)

        reference_and_length = OrderedDict(
            sorted(zip(bam.header.references, bam.header.lengths), key=lambda x: x[0])
        )

        df = df.sort_values(
            by=["read_length", "chrom", "start", "orientation", "count_pos_strand"]
        )
        df = df.rename(columns={"count_pos_strand": "pos", "count_neg_strand": "neg"})
        df_molten = pd.melt(
            df,
            id_vars=[
                "chrom",
        ...
        df_molten = df_molten.sort_values(
            by=[
                "chrom",
                "start",
                "end",
        ...
        df_molten["chrom"] = df_molten.chrom.str.cat(df_molten.strand, sep="__")
        ...
        df_readlen_orient = pd.DataFrame(
            df_molten.groupby(["read_length", "orientation"]).value.agg("sum")
        ).reset_index()
        ...
        assert (
            df_readlen_orient_unmelt["5prime"] == df_readlen_orient_unmelt["3prime"]
        ).all()
        dt = h5py.special_dtype(vlen=str)
        read_lengths_list = df_readlen_orient_unmelt.index.tolist()
        read_lengths_counts = df_readlen_orient_unmelt.loc[
            read_lengths_list, "5prime"
        ].tolist()

        query_alignment_lengths = pd.Series(query_alignment_lengths).sort_index()
        ...
        with tqdm(total=len(df_readlen_grouped)) as pbar:
            with h5py.File("{}.hdf5".format(outprefix), "w") as h5py_file:
                ...
                for read_length, df_readlen_group in df_readlen_grouped:
                    h5_readlen_subgroup = h5_readlen_group.create_group(read_length)
                    df_orientation_grouped = df_readlen_group.groupby("orientation")
                    for orientation, df_orientation_group in df_orientation_grouped:
                        h5_orientation_subgroup = h5_readlen_subgroup.create_group(
                            orientation
                        )
                        df_chrom_grouped = df_orientation_group.groupby("chrom")
                        for chrom, chrom_group in df_chrom_grouped:
                            counts_series = pd.Series(chrom_group["value"].tolist())
                            positions_series = pd.Series(chrom_group["start"].tolist())
                            chrom_group = h5_orientation_subgroup.create_group(chrom)

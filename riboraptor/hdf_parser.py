...
def hdf_to_bigwig(hdf, prefixdir, read_lengths_to_use="all", output_normalized=True):
    ...
    for read_length in read_lengths:
        ...
        for orientation in hdf["fragments"][read_length].keys():
            ...
            mkdir_p(dest_dir)
            bws = {}
            bws["pos_bw"] = os.path.join(
                dest_dir, "{}_{}.bw".format(orientation, "pos")
            )
            bws["neg_bw"] = os.path.join(
                dest_dir, "{}_{}.bw".format(orientation, "neg")
            )
            ...
            if output_normalized:
                ...
                bws["combined_norm_bw"] = os.path.join(
                    dest_dir, "{}_{}.bw".format(orientation, "combined_normalized")
                )
            ...
            for chrom, mapped_strand in chrom_strand:
                ...
                assert mapped_strand in ["pos", "neg"], "Found strand {}".format(
                    mapped_strand
                )
    ...
    for read_length, read_len_group in df_grouped:
        ...
        for orientation, orientation_group in orientation_grouped:
            orientation_group = orientation_group.sort_values(
                by=["chrom", "start", "end"]
            ).drop(columns=["read_length", "orientation"])
            ...
            _create_bigwig_from_bed(
                orientation_group_pos,
                chrom_lengths,
                "{}_{}_{}_{}.bw".format(prefix, read_length, orientation, "pos"),
            )
            _create_bigwig_from_bed(
                orientation_group_neg,
                chrom_lengths,
                "{}_{}_{}_{}.bw".format(prefix, read_length, orientation, "neg"),
            )
    ...
    if strand == "+":
        ...
        for region in intervals_combined:
            ...
            for position in range(region.start, region.end):
                ...
                df.append([region.chrom, position, position + 1, region.strand])
                total += 1
                if total > n_bases:
                    should_stop = True
    else:
...
def create_metagene_from_multi_bigwig(
    bed, bigwigs, max_positions=1000, offset_5p=0, offset_3p=0, n_jobs=16, saveto=None
):
    ...
    for bw in tqdm(bigwigs):
        data = [
            (gene_group, bw.wig_location, offset_5p, offset_3p, max_positions)
...
def merge_bigwigs(bigwigs, chrom_sizes, saveto, scale=False):
    ...
    with tempfile.TemporaryDirectory(dir=TMP_DIR_ROOT) as temp_dir:
        bedgraph = os.path.join(temp_dir, "{}.bedGraph".format(path_leaf(saveto)))
        ...
        cmds = ["bedGraphToBigWig", "{}.sorted".format(bedgraph), chrom_sizes, saveto]
        try:
            p = subprocess.Popen(
                cmds,
                stdout=subprocess.PIPE,
